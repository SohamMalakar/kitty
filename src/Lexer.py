...
class Lexer:
    ...
    def tokenize(self):
        ...
        while self.current_char is not None:
            if self.current_char in self.LETTERS + "_":
            ...
            elif self.current_char in self.WHITESPACE:
                self._skip_whitespace()
            elif self.current_char == "#":
                self._skip_comment()
            elif self.current_char == "+":
                tokens.append(Token(TokenType.PLUS, literal=self.current_char, pos_start=self.position))
                self.advance()
            elif self.current_char == "-":
                tokens.append(self._process_minus_or_arrow())
            elif self.current_char == "*":
                tokens.append(self._process_asterisk_or_power())
            elif self.current_char == "/":
                tokens.append(Token(TokenType.SLASH, literal=self.current_char, pos_start=self.position))
                self.advance()
            elif self.current_char == "%":
                tokens.append(Token(TokenType.MODULUS, literal=self.current_char, pos_start=self.position))
                self.advance()
            elif self.current_char == "=":
    ...
    def _process_asterisk_or_power(self):
